...
10. 문서가 확정되고 사용자가 명시적으로 구현을 요청하면 `start-implementation-thread`를 적용해 별도의 Codex 작업에서 구현합니다.
...
- 구현 승인 후 현재 작업에서 직접 코드를 수정하지 말고 `start-implementation-thread`에 UI 스펙/구현 문서 경로를 전달합니다.
