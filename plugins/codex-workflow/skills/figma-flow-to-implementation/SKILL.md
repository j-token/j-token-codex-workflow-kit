...
description: 사용자가 Figma 링크, 스크린샷, 시각 자료를 제공하거나 UI/화면 구현을 요청할 때 사용합니다. 시각 자료와 기존 앱을 조사해 화면 역할·전이·에셋을 파악하고, 간단한 저위험 UI 수정은 문서 승인 없이 현재 작업에서 구현합니다. 복잡하거나 별도 작업이 필요한 UI는 `setup-codex-prompt`의 단일 프롬프트 문서에 계획과 실행 프롬프트를 합칩니다.
---
...
8. `setup-codex-prompt`로 작업 등급을 정합니다. 빠른 작업은 문서 없이 현재 요청에 따라 구현·검증합니다.
9. 계획·민감 작업은 화면 흐름, 에셋, 상태, 구현 계획, 검증, 모델·추론 강도와 프롬프트를 `.codex/prompts/active/<slug>.md` 한 파일에 기록합니다.
10. 사용자가 계획과 새 작업 시작을 명시적으로 승인하면 `start-implementation-thread`를 적용합니다. 외부 계정 권한이나 비가역 행동은 해당 행동 직전에 별도로 확인합니다.
...
빠른 작업에는 문서를 만들지 않습니다. 계획·민감 작업은 `setup-codex-prompt`의 단일 프롬프트 문서를 사용합니다. 사용자가 UI 스펙 자체를 명시적으로 요청하거나 외부 리뷰 형식이 필요한 경우에만 아래 템플릿을 보조 문서로 사용하고 프롬프트 문서에서 연결합니다. 모든 문서는 `cognitive-writing` 스킬의 원칙을 따라 작성합니다.

파일명은 다음 형식을 사용합니다.

```text
.codex/prompts/supporting/<slug>/ui-spec.md
```
...
- 빠른 작업에서 사용자의 `정리 후 구현해줘`, `이 화면을 수정해줘` 같은 현재 요청은 실행 권한으로 인정하고 같은 작업에서 구현·검증합니다.
- 계획 작업에서 별도 Codex 작업을 만들 때만 계획·모델·추론 강도·프롬프트와 새 작업 시작을 한 번 확인합니다.
