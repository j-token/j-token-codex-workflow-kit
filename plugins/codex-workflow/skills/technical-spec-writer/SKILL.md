...
description: 사용자가 기술 스펙, 구현 스펙, 아키텍처 스펙 또는 IPC/API/SDK/CLI/FFI/JNI/native shell/런타임/빌드/테스트의 장기 공개 계약 문서를 명시적으로 요청했을 때 작성하거나 다듬습니다. 일반 작업은 `setup-codex-prompt`의 단일 프롬프트 문서를 기본으로 하며, 이 스킬을 필수 승인 게이트로 사용하지 않습니다.
---
...
- 기술 스펙은 PRD, 프롬프트 문서 또는 사용자가 직접 확정한 범위를 입력으로 사용할 수 있다.
- 별도 PRD가 없어도 제품 범위와 비범위가 프롬프트 문서나 사용자 요청에서 확인되면 기술 스펙을 작성할 수 있다.
- 장기간 유지할 공개 계약이 아니라 현재 작업 실행만 필요하면 별도 기술 스펙을 만들지 않고 프롬프트 문서에 합친다.

## 작성 원칙

- 스펙은 설명문이 아니라 구현 계약이다.
- 스펙의 목표는 구현 전 불확실성 0건이 아니라 안전한 구현을 시작할 수 있는 최소 충분 계약이다.
- 스펙 작성 직전에 입력 PRD, 프롬프트 문서 또는 사용자 확정 범위를 다시 읽고, 대화의 기억이나 요약만으로 작성하지 않는다.
...
- 스펙 작성 자체를 이유로 현재 턴을 끝내거나 별도 구현 승인을 강제하지 않는다. 빠른 작업은 현재 요청에 따라 계속하고, 새 Codex 작업이 필요한 계획 작업은 프롬프트 문서에서 계획·모델·추론 강도·프롬프트를 한 번 확인한다.

## 작성 흐름

1. 입력 PRD, 프롬프트 문서 또는 사용자가 확정한 범위를 확인하고 다시 읽는다. 실행 계약이 달라지는 범위만 불명확할 때 최소 질문을 한다.
2. 입력을 분류한다.
   - PRD 기반 구현 스펙
   - 아키텍처/런타임 스펙
   - IPC/API/SDK 계약 스펙
...
10. 파일을 저장한 뒤 연결된 프롬프트 문서가 있으면 링크와 상태를 갱신한다. 위험 기반 조건이 맞을 때만 `audit-technical-spec`을 적용한다.

## 출력 위치

- 사용자가 파일 작성을 요청하면 연결된 프롬프트 문서가 있을 때 `.codex/prompts/supporting/<slug>/technical-spec.md`에 저장한다. 연결된 프롬프트 문서가 없으면 `.codex/prompts/supporting/<product>/technical-spec.md`를 사용한다. 사용자가 다른 위치를 명시하면 그 위치를 따른다.
- 보조 문서 경로를 프롬프트 문서에 연결하고 상태를 중복 기록하지 않는다.
...
- [ ] 저장된 기술 스펙을 `audit-technical-spec`에 전달할 경로, 버전과 전체 SHA-256을 내부적으로 확인하고 사용자 표시는 `앞4…뒤4`로 제한했는가
