...
- 기술 스펙 파일을 작성한 직후 `audit-technical-spec`을 적용해 독립된 외부 thread에 구현 가능성, 블로커, 사실 불일치와 계약·검증 공백 감사를 요청한다. 감사 시작에는 별도 구현 승인을 요구하지 않는다.
...
10. 파일을 저장한 뒤 SHA-256을 계산하고 `audit-technical-spec`으로 외부 감사 작업을 시작한다. 외부 감사 작업이 시작되지 않았거나 감사가 완료되지 않았으면 구현 승인 단계로 넘어가지 않는다.
