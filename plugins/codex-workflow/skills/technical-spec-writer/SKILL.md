...
- 기술 스펙 파일을 작성한 직후 `audit-technical-spec`을 적용해 `fork_turns: "none"`인 하위 에이전트에 구현 가능성, 블로커, 사실 불일치와 계약·검증 공백 감사를 요청한다. 감사용 새 thread를 만들지 않으며, 감사 시작에는 별도 구현 승인을 요구하지 않는다.
- 기술 스펙과 감사 작업을 제시한 뒤 현재 턴을 끝낸다. 감사 판정이 `통과`이고 보고서의 스펙 경로, 버전과 SHA-256이 현재 파일과 일치한 뒤, 별도 사용자 메시지에서 명시적으로 구현을 승인한 경우에만 현재 작업에서 직접 코드를 수정하지 말고 `start-implementation-thread`에 기술 스펙과 감사 보고서 경로를 전달한다. 최초 요청의 미래형 구현 의사는 승인으로 인정하지 않는다.
...
10. 파일을 저장한 뒤 SHA-256을 계산하고 `audit-technical-spec`으로 무컨텍스트 하위 에이전트 감사를 시작한다. 감사 에이전트가 시작되지 않았거나 감사가 완료되지 않았으면 구현 승인 단계로 넘어가지 않는다.
