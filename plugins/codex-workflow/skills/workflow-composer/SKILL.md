...
- `audit-technical-spec`: 기술 스펙을 독립된 외부 thread에서 실제 저장소와 공식 자료에 대조하고 구현 게이트 판정을 만듭니다.
...
- 제품·기능 요청은 `요구사항 정리 → PRD 제시 → 별도 기술 스펙 승인 → 기술 스펙 작성 → 외부 thread 독립 감사 → 감사 통과 문서 제시 → 별도 구현 승인` 순서를 지킵니다.
...
- 제품·기능 구현에는 PRD가 아니라 감사가 통과된 기술 스펙과 일치하는 감사 보고서를 전달합니다. `start-implementation-thread`가 두 문서를 다시 읽고 모델과 추론 강도를 선택해 별도의 Codex 작업을 만든 뒤 구현을 넘기게 합니다.
