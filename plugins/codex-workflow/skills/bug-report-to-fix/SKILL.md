...
5. 사용자가 명시적으로 수정/디버깅 시작을 요청하면 `start-implementation-thread`를 적용해 별도의 Codex 작업에서 수정을 시작합니다.
...
사용자가 수정을 승인하면 현재 작업에서 직접 코드를 수정하지 말고 `start-implementation-thread`에 버그 리포트 경로를 전달합니다. 새 작업은 관련 코드를 확인하고 가장 작은 안전한 수정을 적용한 뒤 반드시 검증합니다.
