...
9. 감사 판정이 `통과`이고 감사 보고서의 스펙 경로, 버전과 SHA-256이 현재 기술 스펙과 일치하면 기술 스펙과 감사 보고서를 제시한 뒤 현재 턴을 끝냅니다. 이후 별도 사용자 메시지에서 구현을 승인받은 경우에만 `start-implementation-thread`를 적용합니다.
...
- 감사 판정이 `차단` 또는 `조건부 통과`이면 발견 사항을 기술 스펙에 반영하고 새 SHA-256으로 다시 감사합니다.
- 감사 판정이 `통과`인 기술 스펙과 감사 보고서를 제시한 응답에서는 구현하지 않고 현재 턴을 끝냅니다.
