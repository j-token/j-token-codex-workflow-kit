...
- 채택: 수정·구현·추가·실행 요청은 조사 뒤 같은 작업에서 구현·검증하고, 계획·스펙·프롬프트만 요청한 작업은 `카탈로그 1개 + 작업당 권위 프롬프트 문서 1개`로 관리한다. 문서가 필요한 실행 작업은 구현을 멈추지 않고 함께 갱신한다.
...
flowchart LR
    A["최소 사용자 의도"] --> B{"작업 등급"}
    B -- "실행 요청" --> C["조사 후 현재 작업에서 구현·검증"]
    B -- "계획·스펙·프롬프트 요청" --> D["active 프롬프트 문서 한 파일"]
...
2. 사용자 실행 요청은 복잡도, 파일 수와 스킬 소스 변경 여부에 관계없이 요청과 직접 관련된 변경의 실행 권한으로 인정한다. 조사 중 발견한 별도 버그, 독립 리팩터링과 새 기능은 수정 전에 범위 확장 승인을 받는다.
3. 사용자가 별도 Codex 작업 생성을 명시했을 때만 계획·모델·추론 강도·프롬프트를 한 번 확인한다.
